...
class CustomData implements ArrayAccess {
	...
	public function __construct( array $data = null ) {
		...
		$this->container['content_ids']       = isset( $data['content_ids'] ) ? $this->setContentIds( $data['content_ids'] ) : null;
		$this->container['contents']          = isset( $data['contents'] ) ? $this->setContents( $data['contents'] ) : null;
	...
	public function setContentIds( $content_ids ) {
		if ( ! is_array( $content_ids ) ) {
			$content_ids = str_replace( "'", '"', $content_ids );
	...
	public function setContents( $contents ) {
		if ( ! is_array( $contents ) ) {
			...
			foreach ( $contents as $content ) {
				$contents_object[] = new Content( $content );
			}
	...
	public function getCustomProperty( $key ) {
		...
		throw new PixelException( "Key not found in custom properties:'" . $key . "'" );
	}
	...
	public function normalize() {
		...
		if ( isset( $this->container['custom_properties'] ) ) {
			foreach ( $this->getCustomProperties() as $key => $val ) {
				if ( array_key_exists( $key, $normalized_payload ) ) {
					throw new PixelException( 'Duplicate key defined as part of the custom_properties. key: "' . $key . '". Please make sure the keys defined in the custom_properties are not already available in standard custom_data property list.' );
				}
		...
		return array_filter( $normalized_payload, function ( $val ) {
			if ( is_array( $val ) ) {
				return true;
			}
