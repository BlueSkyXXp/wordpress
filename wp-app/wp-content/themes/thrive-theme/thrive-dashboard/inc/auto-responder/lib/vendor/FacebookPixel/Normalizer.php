...
class Normalizer {
	...
	public static function normalize( $field, $data ) {
		...
		switch ( $field ) {
			case 'em':
				$normalized_data = static::normalizeEmail( $data );
				break;

			case 'ph':
				$normalized_data = static::normalizePhone( $data );
				break;

			case 'zp':
				$normalized_data = static::normalizeZipCode( $data );
				break;

			case 'ct':
				$normalized_data = static::normalizeCity( $data );
				break;

			case 'st':
				$normalized_data = static::normalizeState( $data );
				break;

			case 'country':
				$normalized_data = static::normalizeCountry( $data );
				break;

			case 'currency':
				$normalized_data = static::normalizeCurrency( $data );
				break;

			case 'f5first':
			case 'f5last':
				$normalized_data = static::normalizeF5( $data );
				break;

			case 'fi':
				$normalized_data = static::normalizeFi( $data );
				break;

			case 'dobd':
				$normalized_data = static::normalizeDobd( $data );
				break;

			case 'dobm':
				$normalized_data = static::normalizeDobm( $data );
				break;

			case 'doby':
				$normalized_data = static::normalizeDoby( $data );
				break;

			case 'delivery_category':
				$normalized_data = static::normalizeDeliveryCategory( $data );
				break;

			case 'action_source':
				$normalized_data = static::normalizeActionSource( $data );
				break;
	...
	private static function normalizePhone( $phone ) {
		...
		if ( static::isInternationalNumber( $result ) ) {
	...
	private static function normalizeDobd( $dobd ) {
		...
		if ( ! preg_match( '/^[0-9]{2}$/', $dobd ) ) {
			throw new PixelException( 'Invalid dobd passed(' . $dobd . '). Date of birth day should be in format "DD".' );
		}
