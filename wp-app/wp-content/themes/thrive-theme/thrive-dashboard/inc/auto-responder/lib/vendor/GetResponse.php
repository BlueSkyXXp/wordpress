...
class Thrive_Dash_Api_GetResponse {
	...
	public function getContactsByCustoms( $campaigns = null, $customs, $operator = 'EQUALS' ) {
		...
		if ( ! is_array( $customs ) ) {
			trigger_error( 'Second argument must be an array', E_USER_ERROR );
		}
		foreach ( $customs as $key => $val ) {
			$params['customs'][] = array( 'name' => $key, 'content' => $this->prepTextOp( $operator, $val ) );
		}
		$request  = $this->prepRequest( 'get_contacts', $params );
		$response = $this->execute( $request );
	...
	private function prepTextOp( $operator, $comparison ) {
		if ( ! in_array( $operator, $this->textOperators ) ) {
			trigger_error( 'Invalid text operator', E_USER_ERROR );
		}
